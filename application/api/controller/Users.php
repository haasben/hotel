...
class Users
{
    ...
    public function index()
    {
        ...
        if ($errCode == 0) {
            ...
            $r->expire($key, 3600 * 8);
            //用户基本信息
            $count = db('consumer')->where('openid', $da['openId'])->find();
            if (!$count) {
                $r = db('consumer')->insert(['openid' => $da['openId'], 'username' => $da['nickName'], 'header' => $da['avatarUrl']]);
                $row = db('consumer')->where('openid', $da['openId'])->find();
                if ($r) {
                    $msg = ['error' => 0, 'msg' => '注册成功', 'key' => $key, 'value' => $row];
                } else {
                    $msg = ['error' => 1002, 'msg' => '注册失败'];
                }
            } else {
                $msg = ['error' => 0, 'msg' => '登录成功', 'key' => $key,'value'=>$count];
            }
        }else{
            $msg = ['error' => 1001, 'msg' => '授权失败'];
        }
        return $msg;
    }
    //用户登录
    public function login()
    {
        ...
        $r->connect('127.0.0.1');
        $openid=input('openid');
        $row=db('consumer')->where('is_del',0)->where('openid',$openid)->field('name,')->find();
        if($row){
            if($row['status']==2){
